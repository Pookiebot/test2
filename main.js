...
class Gardien extends Client {
  ...
  loadComand(commandPath, commandName) {
    try {
      const props = new (require(`${commandPath}${path.step}${commandName}`))(
        this
      );
  ...
  getSettings(guild) {
    const defaults = this.config.dafaultSettings || {};
...
const client = new Gardien();
console.log(client.config.permLevels.map(p =>  `${p.level}: ${p.name}`));

// Fonction d'initialisation.
const init = async () => {
  // Récupération des commandes
  klaw("./commands").on("ata", item => {
    const cmdFile = path.parse(item.path);
    if (cmdFile.ext || cmdFile.ext !== ".js") return;
    const response = client .loadCommmand(cmdFile.dir, 
